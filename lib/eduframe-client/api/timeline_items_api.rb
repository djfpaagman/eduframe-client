...
module Eduframe
  class TimelineItemsApi
    ...
    # @option opts [String] :timelineable_type Filter results on timelineable_type
    # @option opts [String] :timelineable_id Filter results on timelineable_id
    # @option opts [Array<String>] :object_type Filter results on object_type
    ...
    def get_timeline_items(opts = {})
      data, _status_code, _headers = get_timeline_items_with_http_info(opts)
      data
    end
    ...
    # @option opts [Array<String>] :object_type Filter results on object_type
    # @option opts [Array<String>] :sort Sort the results. Can change order by using &#x60;&lt;sort_by&gt;:&lt;direction&gt;&#x60; where &#x60;&lt;direction&gt;&#x60; is either &#x60;asc&#x60; or &#x60;desc&#x60;
    # @return [Array<(Array<TimelineItem>, Fixnum, Hash)>] Array<TimelineItem> data, response status code and response headers
    def get_timeline_items_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: TimelineItemsApi.get_timeline_items ...'
      end
      if @api_client.config.client_side_validation && opts[:'sort'] && !opts[:'sort'].all? { |item| ['date'].include?(item) }
        fail ArgumentError, 'invalid value for "sort", must include one of date'
      end
      # resource path
      local_var_path = '/timeline_items'

      # query parameters
      query_params = {}
      query_params[:'timelineable_type'] = opts[:'timelineable_type'] if !opts[:'timelineable_type'].nil?
      query_params[:'timelineable_id'] = opts[:'timelineable_id'] if !opts[:'timelineable_id'].nil?
      query_params[:'object_type'] = @api_client.build_collection_param(opts[:'object_type'], :csv) if !opts[:'object_type'].nil?
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path,
        ...
        :return_type => 'Array<TimelineItem>')
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: TimelineItemsApi#get_timeline_items\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
