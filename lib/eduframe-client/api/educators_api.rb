...
module Eduframe
  class EducatorsApi
    ...
    # @return [Educator]
    def get_educators_current(opts = {})
      data, _status_code, _headers = get_educators_current_with_http_info(opts)
      data
    end
    ...
    # @param [Hash] opts the optional parameters
    # @option opts [Array<String>] :include Include relations
    # @return [Array<(Educator, Fixnum, Hash)>] Educator data, response status code and response headers
    def get_educators_current_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: EducatorsApi.get_educators_current ...'
      end
      if @api_client.config.client_side_validation && opts[:'include'] && !opts[:'include'].all? { |item| ['invoice_address', 'educator_plugins.plugin', 'educator_plugins', 'plugin_data.field.plugin', 'plugin_data.field', 'plugin_data'].include?(item) }
        fail ArgumentError, 'invalid value for "include", must include one of invoice_address, educator_plugins.plugin, educator_plugins, plugin_data.field.plugin, plugin_data.field, plugin_data'
      end
      # resource path
      local_var_path = '/educators/current'
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path,
        ...
        :return_type => 'Educator')
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: EducatorsApi#get_educators_current\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
