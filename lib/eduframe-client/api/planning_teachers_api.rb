...
module Eduframe
  class PlanningTeachersApi
    ...
    # @return [Array<User>]
    def get_planning_teachers(opts = {})
      data, _status_code, _headers = get_planning_teachers_with_http_info(opts)
      data
    end
    ...
    def get_planning_teachers_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: PlanningTeachersApi.get_planning_teachers ...'
      end
      if @api_client.config.client_side_validation && opts[:'include'] && !opts[:'include'].all? { |item| ['labels'].include?(item) }
        fail ArgumentError, 'invalid value for "include", must include one of labels'
      end
      # resource path
      local_var_path = '/planning/teachers'
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: PlanningTeachersApi#get_planning_teachers\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
