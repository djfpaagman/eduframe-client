...
module Eduframe
  class AccountsApi
    ...
    # @option opts [String] :key_contact_user_id Filter results on key_contact_user_id
    # @option opts [String] :user_id Filter results on user_id
    # @option opts [String] :account_type Filter results on account_type
    ...
    # @option opts [Array<String>] :orders_status Filter results on orders_status
    ...
    def get_accounts_with_http_info(opts = {})
      ...
      if @api_client.config.client_side_validation && opts[:'include'] && !opts[:'include'].all? { |item| ['address', 'key_contacts', 'labels', 'signup_answers.signup_question', 'signup_answers', 'plugin_data.field.plugin', 'plugin_data.field', 'plugin_data'].include?(item) }
        fail ArgumentError, 'invalid value for "include", must include one of address, key_contacts, labels, signup_answers.signup_question, signup_answers, plugin_data.field.plugin, plugin_data.field, plugin_data'
      end
      if @api_client.config.client_side_validation && opts[:'sort'] && !opts[:'sort'].all? { |item| ['name', 'users_count'].include?(item) }
        fail ArgumentError, 'invalid value for "sort", must include one of name, users_count'
      end
      # resource path
      ...
      query_params[:'key_contact_user_id'] = opts[:'key_contact_user_id'] if !opts[:'key_contact_user_id'].nil?
      ...
      query_params[:'orders_status'] = @api_client.build_collection_param(opts[:'orders_status'], :csv) if !opts[:'orders_status'].nil?
    ...
    def get_accounts_id_with_http_info(id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: AccountsApi.get_accounts_id ...'
      end
      # verify the required parameter 'id' is set
      if @api_client.config.client_side_validation && id.nil?
        fail ArgumentError, "Missing the required parameter 'id' when calling AccountsApi.get_accounts_id"
      end
      if @api_client.config.client_side_validation && opts[:'include'] && !opts[:'include'].all? { |item| ['address', 'key_contacts', 'labels', 'signup_answers.signup_question', 'signup_answers', 'plugin_data.field.plugin', 'plugin_data.field', 'plugin_data'].include?(item) }
        fail ArgumentError, 'invalid value for "include", must include one of address, key_contacts, labels, signup_answers.signup_question, signup_answers, plugin_data.field.plugin, plugin_data.field, plugin_data'
      end
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path,
        ...
        :return_type => 'Account')
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: AccountsApi#get_accounts_id\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
    # Create an account
    ...
    # @return [Account]
    def post_accounts(opts = {})
      data, _status_code, _headers = post_accounts_with_http_info(opts)
      data
    end
    ...
    # @option opts [Body] :body 
    # @return [Array<(Account, Fixnum, Hash)>] Account data, response status code and response headers
    def post_accounts_with_http_info(opts = {})
      if @api_client.config.debugging
