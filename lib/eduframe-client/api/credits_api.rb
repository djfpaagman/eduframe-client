...
module Eduframe
  class CreditsApi
    ...
    def get_credits_with_http_info(opts = {})
      ...
      if @api_client.config.client_side_validation && opts[:'include'] && !opts[:'include'].all? { |item| ['course', 'enrollment', 'student', 'type.category', 'type'].include?(item) }
        fail ArgumentError, 'invalid value for "include", must include one of course, enrollment, student, type.category, type'
      end
      # resource path
