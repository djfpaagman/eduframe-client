...
module Eduframe
  class CustomersApi
    ...
    def get_customers_with_http_info(opts = {})
      ...
      if @api_client.config.client_side_validation && opts[:'include'] && !opts[:'include'].all? { |item| ['labels', 'address'].include?(item) }
        fail ArgumentError, 'invalid value for "include", must include one of labels, address'
      end
      # resource path
    ...
    # @option opts [Array<String>] :include Include relations
    # @return [User]
    def get_customers_id(id, opts = {})
      data, _status_code, _headers = get_customers_id_with_http_info(id, opts)
      data
    end
    ...
    def get_customers_id_with_http_info(id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: CustomersApi.get_customers_id ...'
      end
      # verify the required parameter 'id' is set
      if @api_client.config.client_side_validation && id.nil?
        fail ArgumentError, "Missing the required parameter 'id' when calling CustomersApi.get_customers_id"
      end
      if @api_client.config.client_side_validation && opts[:'include'] && !opts[:'include'].all? { |item| ['labels', 'address'].include?(item) }
        fail ArgumentError, 'invalid value for "include", must include one of labels, address'
      end
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path,
        ...
        :return_type => 'User')
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: CustomersApi#get_customers_id\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
    # Create a customer.
