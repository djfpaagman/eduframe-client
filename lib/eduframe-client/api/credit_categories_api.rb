...
module Eduframe
  class CreditCategoriesApi
    ...
    # @return [Array<CreditCategory>]
    def get_credit_categories(opts = {})
      data, _status_code, _headers = get_credit_categories_with_http_info(opts)
      data
    end
    ...
    # @param [Hash] opts the optional parameters
    # @option opts [Array<String>] :include Include relations
    # @return [Array<(Array<CreditCategory>, Fixnum, Hash)>] Array<CreditCategory> data, response status code and response headers
    def get_credit_categories_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: CreditCategoriesApi.get_credit_categories ...'
      end
      if @api_client.config.client_side_validation && opts[:'include'] && !opts[:'include'].all? { |item| ['types'].include?(item) }
        fail ArgumentError, 'invalid value for "include", must include one of types'
      end
      # resource path
      local_var_path = '/credit_categories'
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path,
        ...
        :return_type => 'Array<CreditCategory>')
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: CreditCategoriesApi#get_credit_categories\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
