...
module Eduframe
  class EmailTemplatesApi
    ...
    # @option opts [String] :model_class Filter results on model_class
    ...
    def get_email_templates_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: EmailTemplatesApi.get_email_templates ...'
      end
      if @api_client.config.client_side_validation && opts[:'include'] && !opts[:'include'].all? { |item| ['actions'].include?(item) }
        fail ArgumentError, 'invalid value for "include", must include one of actions'
      end
      # resource path
      ...
      query_params[:'model_class'] = opts[:'model_class'] if !opts[:'model_class'].nil?
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: EmailTemplatesApi#get_email_templates\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
