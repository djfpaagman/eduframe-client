...
module Eduframe
  class PlanningLocationsApi
    ...
    # @option opts [String] :available_from Filter results on available_from
    # @option opts [String] :available_to Filter results on available_to
    ...
    # @option opts [String] :course_location_id Filter results on course_location_id
    # @return [Array<MeetingLocation>]
    def get_planning_locations(opts = {})
      data, _status_code, _headers = get_planning_locations_with_http_info(opts)
      data
    end
    ...
    def get_planning_locations_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: PlanningLocationsApi.get_planning_locations ...'
      end
      if @api_client.config.client_side_validation && opts[:'include'] && !opts[:'include'].all? { |item| ['course_location'].include?(item) }
        fail ArgumentError, 'invalid value for "include", must include one of course_location'
      end
      # resource path
      local_var_path = '/planning/locations'

      # query parameters
      query_params = {}
      query_params[:'include'] = @api_client.build_collection_param(opts[:'include'], :csv) if !opts[:'include'].nil?
      query_params[:'available_from'] = opts[:'available_from'] if !opts[:'available_from'].nil?
      query_params[:'available_to'] = opts[:'available_to'] if !opts[:'available_to'].nil?
      ...
      query_params[:'course_location_id'] = opts[:'course_location_id'] if !opts[:'course_location_id'].nil?
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: PlanningLocationsApi#get_planning_locations\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
