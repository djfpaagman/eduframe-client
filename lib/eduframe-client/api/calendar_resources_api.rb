...
module Eduframe
  class CalendarResourcesApi
    ...
    # 
    # @param resources_type Specify the type of resource.
    ...
    # @option opts [Array<String>] :parent_id Filter results on parent_id
    # @return [Array<CalendarResource>]
    def get_calendar_resources_resources_type(resources_type, opts = {})
      data, _status_code, _headers = get_calendar_resources_resources_type_with_http_info(resources_type, opts)
      data
    end
    ...
    # @option opts [String] :minimum_capacity Filter results on minimum_capacity
    # @option opts [Array<String>] :parent_id Filter results on parent_id
    # @return [Array<(Array<CalendarResource>, Fixnum, Hash)>] Array<CalendarResource> data, response status code and response headers
    def get_calendar_resources_resources_type_with_http_info(resources_type, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: CalendarResourcesApi.get_calendar_resources_resources_type ...'
      end
      # verify the required parameter 'resources_type' is set
      if @api_client.config.client_side_validation && resources_type.nil?
        fail ArgumentError, "Missing the required parameter 'resources_type' when calling CalendarResourcesApi.get_calendar_resources_resources_type"
      end
      # verify enum value
      if @api_client.config.client_side_validation && !['teachers', 'materials', 'locations'].include?(resources_type)
        fail ArgumentError, "invalid value for 'resources_type', must be one of teachers, materials, locations"
      end
      # resource path
      local_var_path = '/calendar/resources/{resources_type}'.sub('{' + 'resources_type' + '}', resources_type.to_s)

      # query parameters
      query_params = {}
      query_params[:'search'] = opts[:'search'] if !opts[:'search'].nil?
      query_params[:'minimum_capacity'] = opts[:'minimum_capacity'] if !opts[:'minimum_capacity'].nil?
      query_params[:'parent_id'] = @api_client.build_collection_param(opts[:'parent_id'], :csv) if !opts[:'parent_id'].nil?
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path,
        ...
        :return_type => 'Array<CalendarResource>')
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: CalendarResourcesApi#get_calendar_resources_resources_type\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
