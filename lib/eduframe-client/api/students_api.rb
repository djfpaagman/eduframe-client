...
module Eduframe
  class StudentsApi
    ...
    # @option opts [String] :customer_id *DEPRECATED* Filter results based on the old customer id. Since we don&#39;t have customers anymore in our system, we take the first organisation of the customer. If the user doesn&#39;t have an organisation, we take it&#39;s personal account. 
    # @return [Array<User>]
    ...
    def get_students_with_http_info(opts = {})
      ...
      if @api_client.config.client_side_validation && opts[:'include'] && !opts[:'include'].all? { |item| ['labels', 'signup_answers.signup_question', 'signup_answers'].include?(item) }
        fail ArgumentError, 'invalid value for "include", must include one of labels, signup_answers.signup_question, signup_answers'
      end
      if @api_client.config.client_side_validation && opts[:'sort'] && !opts[:'sort'].all? { |item| ['first_name', 'last_name', 'middle_name'].include?(item) }
        fail ArgumentError, 'invalid value for "sort", must include one of first_name, last_name, middle_name'
      end
      # resource path
    ...
    # @option opts [Array<String>] :include Include relations
    # @return [User]
    def get_students_id(id, opts = {})
      data, _status_code, _headers = get_students_id_with_http_info(id, opts)
      data
    end
    ...
    def get_students_id_with_http_info(id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: StudentsApi.get_students_id ...'
      end
      # verify the required parameter 'id' is set
      if @api_client.config.client_side_validation && id.nil?
        fail ArgumentError, "Missing the required parameter 'id' when calling StudentsApi.get_students_id"
      end
      if @api_client.config.client_side_validation && opts[:'include'] && !opts[:'include'].all? { |item| ['labels', 'signup_answers.signup_question', 'signup_answers'].include?(item) }
        fail ArgumentError, 'invalid value for "include", must include one of labels, signup_answers.signup_question, signup_answers'
      end
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path,
        ...
        :return_type => 'User')
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: StudentsApi#get_students_id\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
  end
