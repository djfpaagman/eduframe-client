...
module Eduframe
  class CreditDefinitionsApi
    ...
    # @return [Array<CreditDefinition>]
    def get_courses_course_id_credit_definitions(course_id, opts = {})
      data, _status_code, _headers = get_courses_course_id_credit_definitions_with_http_info(course_id, opts)
      data
    end
    ...
    # @param [Hash] opts the optional parameters
    # @option opts [Array<String>] :include Include relations
    # @return [Array<(Array<CreditDefinition>, Fixnum, Hash)>] Array<CreditDefinition> data, response status code and response headers
    def get_courses_course_id_credit_definitions_with_http_info(course_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: CreditDefinitionsApi.get_courses_course_id_credit_definitions ...'
      end
      # verify the required parameter 'course_id' is set
      if @api_client.config.client_side_validation && course_id.nil?
        fail ArgumentError, "Missing the required parameter 'course_id' when calling CreditDefinitionsApi.get_courses_course_id_credit_definitions"
      end
      if @api_client.config.client_side_validation && opts[:'include'] && !opts[:'include'].all? { |item| ['type.category', 'type'].include?(item) }
        fail ArgumentError, 'invalid value for "include", must include one of type.category, type'
      end
      # resource path
      local_var_path = '/courses/{course_id}/credit_definitions'.sub('{' + 'course_id' + '}', course_id.to_s)
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path,
        ...
        :return_type => 'Array<CreditDefinition>')
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: CreditDefinitionsApi#get_courses_course_id_credit_definitions\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
