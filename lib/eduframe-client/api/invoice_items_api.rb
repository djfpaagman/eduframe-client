...
module Eduframe
  class InvoiceItemsApi
    ...
    def get_invoices_invoice_id_invoice_items(invoice_id, opts = {})
      data, _status_code, _headers = get_invoices_invoice_id_invoice_items_with_http_info(invoice_id, opts)
      data
    end
    ...
    # @param [Hash] opts the optional parameters
    # @option opts [Array<String>] :include Include relations
    # @return [Array<(Array<InvoiceItem>, Fixnum, Hash)>] Array<InvoiceItem> data, response status code and response headers
    def get_invoices_invoice_id_invoice_items_with_http_info(invoice_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: InvoiceItemsApi.get_invoices_invoice_id_invoice_items ...'
      end
      # verify the required parameter 'invoice_id' is set
      if @api_client.config.client_side_validation && invoice_id.nil?
        fail ArgumentError, "Missing the required parameter 'invoice_id' when calling InvoiceItemsApi.get_invoices_invoice_id_invoice_items"
      end
      if @api_client.config.client_side_validation && opts[:'include'] && !opts[:'include'].all? { |item| ['plugin_data.field.plugin', 'plugin_data.field', 'plugin_data'].include?(item) }
        fail ArgumentError, 'invalid value for "include", must include one of plugin_data.field.plugin, plugin_data.field, plugin_data'
      end
      # resource path
      local_var_path = '/invoices/{invoice_id}/invoice_items'.sub('{' + 'invoice_id' + '}', invoice_id.to_s)
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path,
        ...
        :return_type => 'Array<InvoiceItem>')
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: InvoiceItemsApi#get_invoices_invoice_id_invoice_items\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
